...
```{r filter_time_for_single_poing_with_error, cache=TRUE, eval=TRUE}
library(tibble)
...
```{r stitch_pull_for_single_poing, cache=TRUE, eval=FALSE}
library(tidync)
...
```{r bounding_box_large, cache=TRUE}
library(osmdata)
library(sf)
...
my_boundary_yellow <- opq(bb_manual, timeout=180) %>% 
  add_osm_feature(key = "boundary", value = "national_park") %>% 
osmdata_sf() 
...
```{r plot_of_area_of_interest_large, cache=TRUE, warning=FALSE, fig.height=8}
library(ggplot2)
...
```{r filter_variables_few, cache=TRUE}
input_variables <- inputs$variable_names %>% 
...
```{r pulled_data_for_small_area, cache=TRUE, eval=TRUE}
library(tidync)
...
```{r check_pulled_data_for_large_area, cache=TRUE, fig.height=8, eval=TRUE}
library(ggplot2)
...
```{r stitch_pull_for_large_area, cache=TRUE, eval=TRUE}
library(tidync)
...
```{r rbind_pulled_results_from_large_area, cache=TRUE, eval=TRUE}
...
### Time extent plot
Check this plot to make sure you downloaded your entire time series. 
```{r plot_stitched_data, cache=TRUE, eval=TRUE}
...
```{r rasterize_with_stars, cache=TRUE, fig.height=7}
library(stars)
...
my_boundary <- opq(bb_manual, timeout=180) %>% 
  add_osm_feature(key = "boundary", value = "national_park") %>% 
osmdata_sf() 
...
```{r pulled_bounding_box, cache=TRUE}
library(osmdata)
library(sf)
boundaries <- my_boundary$osm_multipolygons
pulled_bb <- st_bbox(boundaries)
...
```{r plot_of_area_of_interest, cache=TRUE, warning=FALSE, fig.height=8}
library(ggplot2)
basemap <- ggplot(data = boundaries) +
  geom_sf(fill = "cornflowerblue") +
  suppressWarnings(geom_sf_text(aes(label = boundaries$name))) 
...
```{r check_pulled_data, cache=TRUE, fig.height=8, eval=TRUE}
library(ggplot2)
...
```{r pull_river_data, fig.height=10, cache=TRUE}
river <- opq(bb_manual, timeout=180) %>%
...
```{r, eval=TRUE}
library(terra)
roads <- opq(pulled_bb_large, timeout=180) %>%
